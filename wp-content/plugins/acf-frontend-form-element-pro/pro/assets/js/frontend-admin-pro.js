...
(function($){
    ...
    var Field = acf.Field.extend({
        ...
        $currentStep: function(){
            var step = this.$el.attr('data-current-step');
            if( ! step ) step = 1;
            return step;
        },
        $validateSteps: function(){
            return this.$el.attr('data-validate-steps');
        },
        ...
        changeStep: function(step,currentStep){
            ...
            this.$el.attr('data-current-step',step);
            ...
            $('body, html').animate({scrollTop:this.$el.offset().top-100}, 'slow');

            this.$el.find('.acf-loading').addClass('acf-hidden');
