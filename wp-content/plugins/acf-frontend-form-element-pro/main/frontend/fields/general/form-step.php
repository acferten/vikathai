...
class acf_field_form_step extends acf_field {	
			...
			if( $field['type'] != 'form_step' && empty( $field['frontend_step'] ) ){
				...
				if( empty( $steps_wrapper ) ){		
					...
					$steps_wrapper = array_merge( $steps_settings, array(
						'name' => $field['key'] . '_step_wrapper',
						'key'  => $field['key'] . '_step_wrapper',
						'type' => 'form_step',
						'steps_wrapper' => 1,
						'wrapper' => [
							'data-current-step' => 1,
							'data-validate-steps' => $steps_settings['validate_steps']
						],
					) );
	...
	function render_step_tabs( $field ){
		...
		if( ! empty( $field['steps_display'] ) ){
			...
			if( in_array( 'tabs', $field['steps_display'] ) ){
				echo '<div class="frontend-admin-tabs frontend-admin-tabs-view-'.$field['tabs_align'].'"><div class="frontend-admin-tabs-wrapper ' . $tabs_responsive . '">';
	...
	function render_step_fields( $count, $step, $total, $wrapper ){
		?>
		<div class="acf-fields<?php if( $count > 1 ){ echo ' frontend-admin-hidden'; } ?>" data-step="<?php echo $count; ?>">
		<?php
