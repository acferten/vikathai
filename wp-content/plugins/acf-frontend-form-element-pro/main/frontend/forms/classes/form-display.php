...
if( ! class_exists( 'Frontend_WP\Classes\Display_Form' ) ) :
	...
	class Display_Form {
		...
		public function render_fields( $current_fields = array(), $form = array(), $show_button = false, $defaults = false ){
			if( empty( $form ) ) $form = $GLOBALS['admin_form'];
			...
			if( isset( $show_submit_button ) ){
				if( ( $show_button && empty( $GLOBALS['admin_form']['submit_button_field'] ) || isset( $form['approval'] ) ) ){ 
					?>
		...
		public function render_form( $form, $preview = false ) {
			...
			if( isset( $fea_sucess_returned ) ){
				...
				if( empty( $no_message ) ){
					if( isset( $fea_sucess_returned['success_message'] ) && $fea_sucess_returned['location'] == 'current' && $fea_sucess_returned['form_element'] == $form['id'] ){
			...
			$form = $this->render_fields( $form['fields'], $form, true, true );
