        ...
        if ($settings['query_type'] == 'search_page') {
            ...
            if ($settings['relationship_invert']) {
                ...
                switch ($settings['acf_relationship_from']) {
                    case 'current_post':
                        $relations_ids = \get_field($settings['relationship_meta'], get_the_ID());
                        break;
                    case 'current_author':
                        $author_id = get_the_author_meta('ID');
                        $relations_ids = \get_field($settings['relationship_meta'], 'user_' . $author_id);
                        break;
                    case 'current_user':
                        $user_id = get_current_user_id();
                        $relations_ids = \get_field($settings['relationship_meta'], 'user_' . $user_id);
                        break;
                    case 'current_term':
                        $terms = get_the_terms(get_the_ID(), $settings['acf_relation_taxonomy']);
                        if (!empty($terms) && !is_wp_error($terms)) {
                            $relations_ids = \get_field($settings['relationship_meta'], $terms[0]);
                        }
                        break;
                    case 'options_page':
                        $relations_ids = \get_field($settings['relationship_meta'], 'option');
                        break;
                }
            }
            ...
            // Add check for 'object' return type
            if (\is_array($relations_ids) && isset($relations_ids[0]) && \is_object($relations_ids[0])) {
                $relations_ids = Helper::convert_acf_post_objects_to_ids($relations_ids);
            }
            if (!\is_array($relations_ids)) {
                $relations_ids = [$relations_ids];
            } elseif (\is_array($relations_ids[0])) {
